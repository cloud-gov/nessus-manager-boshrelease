...
#check plugin age
PLUGIN_AGE=$(grep 'nessusd-reloader: Reloading nessusd because Plugin auto-update' /var/vcap/store/nessus-manager/opt/nessus/var/nessus/logs/nessusd.messages | tail -n 1| awk -F '[\[\]]' '{print $2}' | { read datefmt ; echo $(( ($(date +%s) - $(date -d "${datefmt}" +%s)) / 86400 )) ; })
